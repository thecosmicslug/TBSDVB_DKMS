...
Next I had been using a build script to recompile the TBS modules after each upgrade. It involved rebooting into a new kernel and running the build script, then reloading the DVB-related modules and restarting services like [TVHeadend](https://tvheadend.org/) depending on them.
...
Now this DKMS module is using the new TBS code which is much quicker to compile and more compatible with other media devices. (previous version would mess up my usb speakers!) 
...
But it did not build all the modules for me (just dvb-core.ko), so I have adapted it to build the required modules for my TBS6285 Quad TV-Tuner PCIe Card. 

Other cards will need to adjust module entries in the `dkms.conf` file. 
